...
//   document.querySelector('.product-imgs').appendChild(dotsContainer);

//   // Создаём точки по количеству слайдов
//   slideItems.forEach((_, i) => {
//       const dot = document.createElement('div');
//       dot.classList.add('dot');
//       dot.addEventListener('click', () => updateActiveImg(i));
...
//       // Обновляем активные классы у миниатюр (если десктоп)
//       slideItems.forEach((item, i) => {
//           item.classList.toggle('active', i === index);
//       });

//       // Обновляем активные точки (если мобильный)
//       dots.forEach((dot, i) => {
//           dot.classList.toggle('active', i === index);
//       });

//       // Меняем главное изображение
//       const imageUrl = slideItems[index]
//           .querySelector('.slide-item__image')
//           .style.backgroundImage.replace(/url\(["']?(.*?)["']?\)/, '$1');
//       mainImage.src = imageUrl;

//       // Обновляем положение слайдера (только для десктопа)
//       if (!isMobile) {
//           slides.style.transform = `translateY(${-index * 83}px)`;
//       }
//   }

//   // Кнопки навигации (только на десктопе)
//   nextBtn.addEventListener('click', () => {
//       if (!isMobile && index < slideItems.length - 1) {
//           updateActiveImg(index + 1);
//       }
//   });
...
//   slideItems.forEach((item, i) => {
//       item.addEventListener('click', () => updateActiveImg(i));
//   });

//   // Проверяем, нужно ли переключаться на точки при изменении размера экрана
//   function checkMobileView() {
//       isMobile = window.innerWidth <= 768;
//       dotsContainer.style.display = isMobile ? 'flex' : 'none';
//       slides.style.display = isMobile ? 'none' : 'flex';
...
document.addEventListener("DOMContentLoaded", function() {
  document.addEventListener("scroll", () => {
      ...
      if (!productItem || !floatingActions) return;
